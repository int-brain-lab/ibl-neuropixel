...
class TestEphysSpikeSortingMultiProcess(unittest.TestCase):
    ...
    def test_parallel_computation(self):
        ...
        voltage.decompress_destripe_cbin(
            self.file_path, out_file, nprocesses=1, batch_size=6556
        )
        ...
        shutil.copy(self.meta_file, out_file.with_suffix(".meta"))
        voltage.decompress_destripe_cbin(
            self.file_path, out_file, nprocesses=4, batch_size=6556
        )
        ...
        ns2add = 100
        voltage.decompress_destripe_cbin(
            self.file_path, out_file, nprocesses=4, batch_size=6556, ns2add=ns2add
        )
        ...
        shutil.copy(self.meta_file, out_file.with_suffix(".meta"))
        voltage.decompress_destripe_cbin(
            self.file_path, out_file, nprocesses=4, batch_size=6556, wrot=wm
        )
        ...
        shutil.copy(self.meta_file, out_file.with_suffix(".meta"))
        voltage.decompress_destripe_cbin(
            self.file_path, out_file, nprocesses=4, batch_size=6556, append=True
        )
