...
- `ibldsp.voltage.destripe`: Now the sample shift information is first checked in the geometry metdata, then it tries to pull the sample shift info based on the neuropixel version and the number of shanks, else it does not apply any sample shifitng but simply raises a warning.
...
- `spikeglx.Reader.decompress`: supports an explicit *.ch file argument, also will search for matching pattern with UUID in file name
