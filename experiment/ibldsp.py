...
class TestBadChannelDetection:
    def __init__(self):
        ...
        try:
            ...
            self.filepath_save = (
                Path(os.getcwd()).joinpath("ibl-neuropixel").joinpath("experiment")
            )
            if not self.filepath_save.is_dir():
                self.filepath_save.mkdir()

        self.filename_save = "bad_ch_annotations"
        self.annotations_path = self.filepath_save.joinpath(
            f"{self.filename_save}.json"
        )
        ...
        if self.annotations_path.is_file():
            ...
            self.annotations = {
                ...
                "4cb60c5c-d15b-4abd-8cfd-776bc5a81dbe": {
                    "idead": [16, 29, 191, 357],
                    "inoisy": [],
                    "ioutside": [
                        358,
    ...
    def test_bad_channel_detection(self):
        def detect_bad_chan(pid, one=None, t0=100, duration=1.0, display=False):
            ...
            first, last = (
                int(window_secs_ap[0] * sr_ap.fs) + s_event,
                int(window_secs_ap[1] * sr_ap.fs + s_event),
            )
            raw_ap = sr_ap[first:last, : -sr_ap.nsync].T
            # Detect bad channels
            channel_labels, _ = detect_bad_channels(
                raw_ap, fs=sr_ap.fs, display=display
            )
